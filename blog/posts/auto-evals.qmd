...
A caveat before the numbers. This experiment is one dataset, one prompt, and 39 labeled failures, so the differences between systems come down to a handful of traces. They cannot establish that one vendor is better than another, and ranking vendors was not the goal.[^neutral] We share the numbers for transparency. The interesting part is what every system missed.

## What the tools caught

| System | Recall | Precision | Discoveries | Wrong flags |
|---|---|---|---|---|
...
Claude Code, Codex, and Factory Droid are general purpose coding agents and were competitive with the platforms. They received the same prompt and the same masked traces. Codex quoted exact tool arguments, dates, prices, and conflicting values in its evidence. It caught subtle failures the platforms missed, including an in-person tour link presented as confirmation of a virtual tour. Droid matched Codex's recall with fewer wrong flags, though it never rated anything critical, even failures that deserved it.
...
**👉 _Want to learn more about Evals? Check out our [AI Evals course](https://bit.ly/evals-ai){target="_blank"}_**. It's a live cohort with hands on exercises and office hours. Here is a [25% discount code](https://bit.ly/evals-ai){target="_blank"} for readers. 👈
:::

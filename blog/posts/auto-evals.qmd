...
New products nudge the other way. Braintrust, Arize, and LangSmith have each shipped an agent (Loop, Alyx, and Engine) that reads your production traces and reports what is failing.[^tools] Coding agents have become capable enough to attempt the same job from a folder of log files or traces. We believe in using AI in any workflow where it helps, including error analysis. But the temptation with these tools is to take the human out of the loop entirely. How much can these systems catch on their own, and what do they miss?
...
A caveat before the numbers. This experiment is one dataset, one prompt, and 39 labeled failures, so the differences between systems come down to a handful of traces. They cannot establish that one vendor is better than another, and ranking vendors was not the goal.[^neutral] We set out to find where these systems systematically fall short, and to learn what they were like to use.
...
The chat agent's strength was evidence. When it flagged a real failure, it usually cited the exact message or tool call, which made its findings the easiest to verify. Its weakness was severity ratings. Several of its wrong findings were marked high severity, and inflated severity misleads a team deciding what to fix first.
...
**👉 _Want to learn more about Evals? Check out our [AI Evals course](https://maven.com/parlance-labs/evals?promoCode=evals-faq-new){target="_blank"}_**. It's a live cohort with hands on exercises and office hours. Here is a [25% discount code](https://maven.com/parlance-labs/evals?promoCode=evals-faq-new){target="_blank"} for readers. 👈
:::
